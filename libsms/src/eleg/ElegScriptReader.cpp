...
ElegScriptReader::ElegScriptReader(
...
bool ElegScriptReader::operator()() {
  while (!src.eof()) {
    std::string line;
    src.getLine(line);
    ++lineNum;
...
void ElegScriptReader::loadThingy(const BlackT::TThingyTable& thingy__) {
  thingy = thingy__;
}
  
void ElegScriptReader::outputNextSymbol(TStream& ifs) {
  // literal value
  if ((ifs.remaining() >= 5)
      && (ifs.peek() == '<')) {
    int pos = ifs.tell();
...
//    std::cerr << std::dec << lineNum << " " << std::hex << result.id << " " << result.size << std::endl;
  ...
  throw TGenericException(T_SRCANDLINE,
                          "ElegScriptReader::outputNextSymbol()",
                          "Line "
...
void ElegScriptReader::flushActiveScript() {
  // write terminator
...
void ElegScriptReader::resetScriptBuffer() {
...
/*bool ElegScriptReader::checkSymbol(BlackT::TStream& ifs, std::string& symbol) {
...
void ElegScriptReader::processDirective(BlackT::TStream& ifs) {
  ...
  else if (name.compare("ENDBLOCK") == 0) {
    return;
  }
  else {
    throw TGenericException(T_SRCANDLINE,
                            "ElegScriptReader::processDirective()",
                            "Line "
                              + TStringConversion::intToString(lineNum)
                              + ":\n  Unknown directive: "
                              + name);
...
void ElegScriptReader::processLoadTable(BlackT::TStream& ifs) {
  std::string tableName = TParse::matchString(ifs);
  TThingyTable table(tableName);
  loadThingy(table);
}

void ElegScriptReader::processStartMsg(BlackT::TStream& ifs) {
...
void ElegScriptReader::processEndMsg(BlackT::TStream& ifs) {
  flushActiveScript();
}

void ElegScriptReader::processIncBin(BlackT::TStream& ifs) {
...
void ElegScriptReader::processBreak(BlackT::TStream& ifs) {
  breakTriggered = true;
}
