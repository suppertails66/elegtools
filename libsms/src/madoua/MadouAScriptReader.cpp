...
MadouAScriptReader::MadouAScriptReader(
...
bool MadouAScriptReader::operator()() {
  try {
    while (!src.eof()) {
      std::string line;
      src.getLine(line);
...
void MadouAScriptReader::loadThingy(const BlackT::TThingyTable& thingy__) {
  thingy = thingy__;
}
  
void MadouAScriptReader::outputNextSymbol(TStream& ifs) {
  // literal value
  if ((ifs.remaining() >= 5)
      && (ifs.peek() == '<')) {
    int pos = ifs.tell();
...
//    std::cerr << std::dec << lineNum << " " << std::hex << result.id << " " << result.size << std::endl;
  ...
  throw TGenericException(T_SRCANDLINE,
                          "MadouAScriptReader::outputNextSymbol()",
                          "Line "
...
void MadouAScriptReader::flushActiveScript() {
  // write terminator
...
void MadouAScriptReader::resetScriptBuffer() {
...
/*bool MadouAScriptReader::checkSymbol(BlackT::TStream& ifs, std::string& symbol) {
...
void MadouAScriptReader::processDirective(BlackT::TStream& ifs) {
  ...
  if (name.compare("LOADTABLE") == 0) {
    ...
    throw TGenericException(T_SRCANDLINE,
                            "MadouAScriptReader::processDirective()",
                            "Line "
                              + TStringConversion::intToString(lineNum)
                              + ":\n  Unknown directive: "
                              + name);
...
void MadouAScriptReader::processLoadTable(BlackT::TStream& ifs) {
  std::string tableName = TParse::matchString(ifs);
  TThingyTable table(tableName);
  loadThingy(table);
}

void MadouAScriptReader::processStartMsg(BlackT::TStream& ifs) {
...
void MadouAScriptReader::processEndMsg(BlackT::TStream& ifs) {
  flushActiveScript();
}

void MadouAScriptReader::processIncBin(BlackT::TStream& ifs) {
...
void MadouAScriptReader::processBreak(BlackT::TStream& ifs) {
  breakTriggered = true;
}

void MadouAScriptReader::processEndRegion(BlackT::TStream& ifs) {
  TParse::matchInt(ifs);
//  flushActiveScript();
  breakTriggered = true;
}
