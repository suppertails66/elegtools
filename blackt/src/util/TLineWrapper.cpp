...
//      std::cerr << "start: " << xPos << std::endl;
      // output current word
      flushActiveWord();
//      std::cerr << "next: " << xPos << std::endl;
...
void TLineWrapper::flushActiveWord() {
  ...
  if ((xSize == -1) || ((xPos + currentWordWidth) <= xSize)) {
    ...
    if ((xPos + currentWordWidth) > xSize) {
      ...
      std::cerr << "  Max width " << xSize
        << ", actual width " << xPos + currentWordWidth
        << ", overflow " << ((xPos + currentWordWidth) - xSize) << std::endl;
//      currentWordBuffer.seek(0);
//      while (!currentWordBuffer.eof()) {
